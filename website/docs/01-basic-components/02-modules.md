...
Окрім `DynamicModule`, в метадані модуля також можна передавати об'єкт з типом `DynamicModuleWrapper`:
...
Ця обгортка є критично важливою, коли ви імпортуєте **один і той самий** динамічний модуль у кількох аспектах і потребуєте різних налаштувань для кожного. Приклад декларативного використання:
...
:::warning
Зверніть увагу: при використанні `DynamicModuleWrapper` динамічні опції мерджаться безпосередньо у базовий об'єкт `DynamicModule`. 
Тому, ви **можете** використовувати одне й те саме посилання на об'єкт `DynamicModule` у декількох обгортках **тільки при імпорті в один і той самий модуль-споживач** (наприклад, для конфігурації різних аспектів, застосованих до цього ж модуля). 
Проте ви **не можете** використовувати те саме посилання на об'єкт `DynamicModule` для імпорту в **різні модулі-споживачі**. В такому випадку опції з обгорток одного модуля "перетечуть" (зіллються) у імпорти іншого модуля глобально. Для імпорту в інший модуль-споживач необхідно створювати **новий** екземпляр об'єкта `DynamicModule` (наприклад, за допомогою виклику статичного методу `SomeModule.forRoot()`).
:::
